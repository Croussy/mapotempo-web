...
class ImporterDestinations < ImporterBase
  ...
  def after_import(replace, name, synchronous)
    ...
    if @routes.size > 1 || !@routes.key?(nil)
      @planning = @customer.plannings.build(name: name || I18n.t('activerecord.models.planning') + ' ' + I18n.l(Time.now, format: :year_month_day_hour_minute), vehicle_usage_set: @customer.vehicle_usage_sets[0], tags: @common_tags || [])
